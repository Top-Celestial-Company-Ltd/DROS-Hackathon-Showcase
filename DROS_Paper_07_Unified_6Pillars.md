...
The rapid proliferation of Large Language Model (LLM) agents equipped with Function Calling and Tool Invocation capabilities has exposed a critical governance gap in enterprise IT infrastructure. Unlike traditional software with deterministic, pre-compiled control flows, autonomous agents generate dynamic execution paths at runtime. When deployed in enterprise environments—interacting with SAP ERPs, Core Banking APIs, Hospital EHR databases, or Government MyData portals—an ungoverned agent poses severe operational risks, including prompt injection, privilege escalation, cross-agency data exfiltration, and unauthorized contractual commitments.

### 1.2 Throughput Imperative & Denial-of-Service Risk in High-Frequency Agent Execution
In an enterprise deployment, autonomous agents generate high-frequency streams of system calls (Syscalls) and FFI tool invocations across internal, partner, and external networks. Under peak operational loads or malicious exploitation (e.g., recursive tool invocation attacks), an agent workload can issue thousands of requests per second. If the runtime governance kernel introduces millisecond-level evaluation overhead (e.g., out-of-band network calls or JSON policy evaluation), the governance layer itself becomes the primary throughput bottleneck, triggering system-wide latency amplification and self-induced Denial-of-Service (DDoS) failure.

To prevent governance-induced system collapse while maintaining strict compliance, the evaluation plane must operate within a microsecond-level latency budget. DROS-6P bounds its decision latency at **$26.1\ \mu\text{s}$**, enabling real-time, in-band policy enforcement capable of absorbing high-throughput agent Syscall streams without degrading underlying service availability.
