...
* **【組件 A】DIT Token（身分憑證注入層）**
  * 密碼學強綁定 Agent ID ＋ 法人(vLEI) / 自然人(MyData) 雙軌授權
  * 精確鎖定 Capability Bitmaps 權限位元圖與到期時間
* 　　⬇ *(帶內函式調用鏈)*
* **【組件 B】VEP Policy Gate（帶內執行期策略閘門層）**
  * **26.1 μs** C-ABI 帶內二進位硬熔斷（HTTP 403 FORBIDDEN）
  * 高風險動作自動掛起交易，觸發 HITL 人類雙重簽署
* 　　⬇ *(確定性執行與存證)*
* **【組件 C】Merkle Audit Chain（不可竄改稽核追蹤層）**
  * SHA-256 Merkle Hash 鏈結密碼學證據鏈
  * 產出具備法庭採信力的不可否認性存證收據
...
| **5. 存取稽核 (Auditing)** | uditd / Linux Journal 日誌 | **SHA-256 Merkle Hash 密碼學證據鏈** | 解決「事後偽造與串供，產出法院採信收據」 |
| **6. 資源回收 (Revocation)** | kill -9 / Process Terminate | **(1)$ RCU 原子指針秒級動態撤銷** | 解決「授權過期或被撤銷後，背景 Agent 偷跑」 |
...
2. **📤 Egress 邊界（確定性動作輸出 / Execution Sink）**：確保 AI Agent 發起的每一次 Tool Call / Action，**物理上必須通過 C-ABI 帶內策略閘門**，在 $2626.1 μs 內完成 6P 閉環校驗並生成具備法院採信力之 Merkle 存證，未授權動作瞬間硬熔斷。事後偽造與串供，產出法院採信收據」 |
...
2. **📤 接口 ②：Execution Sink（確定性動作與工具執行接口 / Safe Egress）**：在 C-ABI 二進位層級於 $2626.1 μs 內完成 6P 帶內安全檢驗與 Merkle 密碼學存證。
